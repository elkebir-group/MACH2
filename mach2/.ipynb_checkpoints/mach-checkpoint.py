...
from MACH2.clonetree import CloneTree, RefinedCloneTree
from MACH2.migrationgraph import MigrationGraph
from MACH2.solutionset import SolutionSet


class MACH:

    def __init__(self, clone_tree, suboptimal_mode=False, seeding_site=False, specify_migration_comigration=None, possible_migration_list=None):
        ...
        if clone_tree.primary_site is not None:
    ...
    def add_primary_site_constraints(self):
        ...
        self.m.constraints_primary.add(self.m.r[self.clone_tree.primary_site] == 1)
        ...
        if solver == 'gurobi':
            ...
            for soln in range(n_actual_solutions):
                ...
                for s_1 in self.Sigma:
                    for s_2 in self.Sigma:
                        if opt._pyomo_var_to_solver_var_map[self.m.z[s_1, s_2]].Xn > 0.5:
                            ...
                            gamma += opt._pyomo_var_to_solver_var_map[self.m.z[s_1, s_2]].Xn
                        # for l in self.L:
                ...
                solution_raw = {'vertex_multilabeling': ell,
                                'aug_mig_graph': G, 'n_mig': mu, 'n_comig': gamma}
...
if __name__ == '__main__':
    ...
    solver = MACH(clone_tree, suboptimal_mode=args.suboptimal, seeding_site=args.seeding_sites)
    solver.solve('gurobi', 100)
    ...
    #             soln.write_G(e, f'{output_str}/{primary_str}-G-{str(e).zfill(padding)}.graph')
